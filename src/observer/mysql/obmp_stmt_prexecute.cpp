...
int ObMPStmtPrexecute::response_fail_result(sql::ObSQLSessionInfo &session, int err_ret)
{
  ...
  arraybinding_row_->get_cell(arraybinding_row_->get_count() - 1).set_varchar(ob_errpkt_strerror(err_ret));
  if (OB_FAIL(response_row(session, *arraybinding_row_, arraybinding_columns_, false))) {
    LOG_WARN("fail to response fail row to client", K(ret));
  }
  return ret;
...
//  session.get_trans_desc().consistency_wait(); // sync timestamp
  ...
  if (OB_ISNULL(fields) || NULL == arraybinding_row_ || arraybinding_row_->get_count() < 3) {
    ...
    if (OB_ITER_END == ret) {
      ...
      arraybinding_row_->get_cell(arraybinding_row_->get_count() - 1).set_varchar(ob_errpkt_strerror(ret));
