...
public:
  ...
  public:
    ObBuildDDLErrorMessage()
      : ret_code_(common::OB_NOT_INIT), ddl_type_(ObDDLType::DDL_INVALID), affected_rows_(0),
        user_message_(nullptr), dba_message_("\0"), allocator_(), published_schema_version_(OB_INVALID_VERSION)
    {}
    ...
    bool operator!=(const ObBuildDDLErrorMessage &other) const;
    TO_STRING_KV(K_(ret_code), K_(ddl_type), K_(affected_rows), K_(user_message), K_(dba_message), K_(published_schema_version));
  public:
    int ret_code_;
    ObDDLType ddl_type_;
  ...
  static int get_ddl_error_message(const int64_t task_id,
      const int64_t target_object_id,
      const int64_t object_id,
      common::ObMySQLProxy &sql_proxy,
      ObBuildDDLErrorMessage &error_message,
      int64_t &forward_user_msg_len);
  static int report_ddl_error_message(const ObBuildDDLErrorMessage &error_message,
  ...
  static int build_ddl_error_message(const int ret_code, const uint64_t table_id,
