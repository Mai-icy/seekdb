...
public:
  ...
  ObTenantCatalogId(const uint64_t catalog_id)
    : ObTenantCommonSchemaId(catalog_id) {}
};
...
public:
  ...
  inline ObTenantCatalogId get_tenant_catalog_id() const { return ObTenantCatalogId(catalog_id_); }
  ...
  virtual void reset();
  TO_STRING_KV(K_(catalog_id), K_(schema_version), K_(catalog_name),
...
{
  ObCatalogPrivSortKey() : user_id_(common::OB_INVALID_ID)
  {}
  ObCatalogPrivSortKey(const uint64_t user_id,
                       const common::ObString &catalog)
      : user_id_(user_id), catalog_(catalog)
  {}
  ...
  TO_STRING_KV(K_(user_id), K_(catalog));
  uint64_t user_id_;
...
public:
  ...
  { return ObCatalogPrivSortKey(user_id_, catalog_); }
  ...
  TO_STRING_KV(K_(user_id), K_(catalog), "privileges", ObPrintPrivSet(priv_set_));
  //other methods
  virtual bool is_valid() const;
  virtual void reset();
  int64_t get_convert_size() const;
