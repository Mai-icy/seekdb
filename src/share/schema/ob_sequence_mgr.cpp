...
{
  Get_Sequence_Filter(uint64_t db_id)
      : db_id_(db_id) {}
  bool operator() (ObSequenceSchema *value)
  {
...
int ObSequenceMgr::get_sequence_schema_with_name(const uint64_t database_id,
  const ObString &name,
  const ObSequenceSchema *&sequence_schema) const
{
  ...
  if (!is_inited_) {
    ...
    ObSequenceHashWrapper hash_wrap(database_id, name);
    if (OB_FAIL(sequence_map_.get_refactored(hash_wrap, tmp_schema))) {
      if (OB_HASH_NOT_EXIST == ret) {
        ret = OB_SUCCESS;
        LOG_INFO("sequence is not exist", K(database_id), K(name),
...
int ObSequenceMgr::del_sequence(const ObTenantSequenceId &sequence)
{
  ...
  if (!sequence.is_valid()) {
    ...
    ObSequenceHashWrapper sequence_wrapper(schema_to_del->get_database_id(),
...
int ObSequenceMgr::get_sequence_schemas_in_database(const uint64_t database_id, ObIArray<const ObSequenceSchema *> &sequence_schemas) const
{
  int ret = OB_SUCCESS;
  sequence_mgr::Get_Sequence_Filter filter(database_id);
...
int ObSequenceMgr::get_sequence_schemas_in_tenant(ObIArray<const ObSequenceSchema *> &sequence_schemas) const
{
  int ret = OB_SUCCESS;
