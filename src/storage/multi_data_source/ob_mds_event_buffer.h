...
#ifndef OB_STORAGE_MULTI_DATA_SOURCE_MDS_EVENT_BUFFER_H_
#define OB_STORAGE_MULTI_DATA_SOURCE_MDS_EVENT_BUFFER_H_
...
struct MdsEvent {
  ...
  TO_STRING_KV(KP_(alloc), KTIME_(timestamp), K_(obj_ptr), K_(event), K_(info_str), K_(unit_id), K_(key_str), \
               ...
               K_(state));
public:
  // Diagnostic snapshot fields are intentionally readable by upper-layer
  // virtual-table adapters. Mutation remains owned by MDS.
  ...
  uint32_t thread_id() const { return tid_; }
  const char *thread_name() const { return tname_; }
  const ObCurTraceId::TraceId &trace_id() const { return trace_id_; }
  int64_t timestamp() const { return timestamp_; }
  const char *event_name() const { return event_; }
  const ObString &info() const { return info_str_; }
  const ObString &key() const { return key_str_; }
  storage::mds::WriterType writer_type() const { return writer_type_; }
  int64_t writer_id() const { return writer_id_; }
  const transaction::ObTxSEQ &seq_no() const { return seq_no_; }
  const share::SCN &redo_scn() const { return redo_scn_; }
  const share::SCN &end_scn() const { return end_scn_; }
  const share::SCN &trans_version() const { return trans_version_; }
  storage::mds::MdsNodeType node_type() const { return node_type_; }
  storage::mds::TwoPhaseCommitState state() const { return state_; }
private:
...
#endif // OB_STORAGE_MULTI_DATA_SOURCE_MDS_EVENT_BUFFER_H_
