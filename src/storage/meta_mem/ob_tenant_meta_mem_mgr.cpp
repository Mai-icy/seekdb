...
int ObTenantMetaMemMgr::mtl_new(ObTenantMetaMemMgr *&meta_mem_mgr)
{
  ...
  meta_mem_mgr = OB_NEW(ObTenantMetaMemMgr, ObMemAttr("MetaMemMgr"));
...
int ObTenantMetaMemMgr::gc_tables_in_queue(bool &all_table_cleaned)
{
  ...
  // no matter success or failure, print GC result
  {
    ...
    if (OB_NOT_NULL(alloc)) {
      const ObTenantCtxAllocatorGuard &ag = alloc->get_tenant_ctx_allocator(ObCtxIds::META_OBJ_CTX_ID);
      if (OB_NOT_NULL(ag)) {
                                                           ...
                                                           SCN &min_end_scn)
{
  ...
  if (OB_ISNULL(tablet)) {
    ...
    if (OB_EMPTY_RESULT == ret) {
      // When OB_EMPTY_RESULT is returned, the tablet may have just been created and
      // committed its tablet status. The old version tablet is not readable in this
      // case, so it is safe to return max scn here.
      min_end_scn.set_max();
      ret = OB_SUCCESS;
    } else {
    ...
    if (!user_data.reserved_scn_.is_valid()) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("reserved_scn is invalid", K(ret), K(user_data));
    } else {
      min_end_scn = SCN::scn_dec(user_data.reserved_scn_);
    }
  } else {
    ...
    bool &allow_tablet_version_gc)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_FAIL(tablet_map_.get(key, ptr_handle))) {
      ...
      const ObDieingTabletMapKey dieing_key(key);
...
int ObTenantMetaMemMgr::register_into_tb_map(const char *file, const int line,
  ...
  int ret = OB_SUCCESS;
  ObTenantMetaMemMgr *t3m = OB_NOT_NULL(share::g_mp) ? share::g_mp->tenant_meta_mem_mgr() : nullptr;
...
int ObTenantMetaMemMgr::inc_external_tablet_cnt(const uint64_t tablet_id)
{
  int ret = OB_SUCCESS;
...
int ObTenantMetaMemMgr::dec_external_tablet_cnt(const uint64_t tablet_id)
{
  int ret = OB_SUCCESS;
  const ObDieingTabletMapKey dieing_key(tablet_id);
...
{
  int ret = OB_SUCCESS;
  ObMemAttr attr = SET_USE_500(ObMemAttr("TabletIterSE"));
