                         ...
                         ObXATransID xid)
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ...
    TRANS_LOG(WARN, "invalid argument", K(trans_id), K(cluster_version),
              KP(trans_service));
    ...
    if (OB_FAIL(lock_.init(this))) {
      ...
      TRANS_LOG(WARN, "ObTxCtx init memtable context error", KR(ret), K(trans_id));
    } else if (OB_FAIL(init_log_cbs_(trans_id))) {
      TRANS_LOG(WARN, "init log cbs failed", KR(ret), K(trans_id));
    } else if (OB_FAIL(ctx_tx_data_.init(trans_expired_time, ls_ctx_mgr, trans_id))) {
      TRANS_LOG(WARN, "init ctx tx data failed",K(ret), K(trans_id));
    } else if (OB_FAIL(mds_cache_.init(trans_id))) {
      TRANS_LOG(WARN, "init mds cache failed", K(ret), K(trans_id));
    }
  }
...
int ObTxCtx::init_memtable_ctx_()
{
  ...
  if (OB_FAIL(mt_ctx_.init())) {
    ...
    TRANS_LOG(WARN, "enable_lock_table fail", K(ret));
  } else {
  ...
  TRANS_LOG(DEBUG, "init memtable ctx", KR(ret), K(trans_id_));
  return ret;
}
        ...
        if (ObTxState::REDO_COMPLETE == get_downstream_state()) {
          if (!is_logging_()) {
            if (OB_FAIL(one_phase_commit_())) {
              TRANS_LOG(WARN, "one phase commit failed", KR(ret), KPC(this));
            } else {
...
int ObTxCtx::gc_ctx_()
{
  ...
  if (OB_FAIL(prepare_mul_data_source_tx_end_(false))) {
    TRANS_LOG(WARN, "trans gc need retry", K(ret), K(trans_id_));
  } else {
...
int ObTxCtx::recover_tx_ctx_table_info(ObTxCtxTableInfo &ctx_info)
{
  ...
  if (!ctx_info.is_valid()) {
    ...
    if (OB_FAIL(ret)) {
      ...
      if (OB_SUCC(ret)) {
        TRANS_LOG(INFO, "recover retain ctx into mgr success", K(ret), K(trans_id_));
      }
    }
...
int ObTxCtx::compensate_abort_log_()
{
  ...
  if (is_force_abort_logging_()) {
    ...
    if (OB_TMP_FAIL(restart_commit_retry_timer_())) {
      TRANS_LOG(WARN, "restart_commit_retry_timer_ for submit abort log fail",
  ...
  if (retry_submit_mds) {
    ...
    if (OB_TMP_FAIL(submit_log_impl_(ObTxLogType::TX_MULTI_DATA_SOURCE_LOG))) {
      TRANS_LOG(WARN, "submit mds log failed in on_success", K(ret), K(tmp_ret), K(trans_id_),
                K(exec_info_.state_), K(target_state_), K(ctx_tx_data_), K(mds_cache_),
...
int ObTxCtx::gts_callback_interrupted()
{
  ...
  if (need_revert_ctx) {
    if (OB_FAIL(ls_tx_ctx_mgr_->revert_tx_ctx_without_lock(this))) {
      TRANS_LOG(ERROR, "release ctx ref failed", KR(ret));
    }
  }
  return ret;
}
...
int ObTxCtx::wait_gts_elapse_commit_version_(bool &need_wait)
{
  ...
  if (OB_FAIL(trans_service_->get_tx_timestamp_waiter().wait_gts_elapse(
    ...
    TRANS_LOG(WARN, "wait gts elapse failed", KR(ret), KPC(this));
  } else if (need_wait) {
    runtime_state_.set_gts_waiting();
    if (OB_FAIL(acquire_ctx_ref_())) {
      TRANS_LOG(WARN, "get trans ctx ref failed", KR(ret), KPC(this));
    }
...
int ObTxCtx::submit_direct_load_inc_log_(
    DLI_LOG &dli_log,
    ...
    bool need_free_extra_cb)
{
  ...
  TRANS_LOG(DEBUG, "<ObTxDirectLoadIncLog> submit direct load inc log", K(ret), K(get_trans_id()),
            K(dli_log), K(dli_log_type), K(batch_key), K(replay_barrier),
            K(replay_hint), K(scn));
  return ret;
}

int ObTxCtx::submit_direct_load_inc_redo_log(storage::ObDDLRedoLog &ddl_redo_log,
                                                    logservice::AppendCb *extra_cb,
                                                    const int64_t replay_hint,
                                                    share::SCN &scn)
{
  int ret = OB_SUCCESS;
  common::ObTimeGuard timeguard("ObTxCtx::submit_direct_load_inc_redo_log",
                                1 * 1000 * 1000); // 1s
...
int ObTxCtx::submit_direct_load_inc_start_log(storage::ObDDLIncStartLog &ddl_start_log,
                                                     logservice::AppendCb *extra_cb,
                                                     share::SCN &scn)
{
  int ret = OB_SUCCESS;
...
int ObTxCtx::submit_direct_load_inc_commit_log(storage::ObDDLIncCommitLog &ddl_commit_log,
                                                      logservice::AppendCb *extra_cb,
                                                      share::SCN &scn,
                                                      bool need_free_extra_cb)
{
...
int ObTxCtx::check_dli_batch_completed_(ObTxLogType submit_log_type)
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  ...
  if (exec_info_.dli_batch_set_.size() > 0) {
    ...
    for (ObDLIBatchSet::iterator iter = exec_info_.dli_batch_set_.begin();
      ...
      if (iter->first.need_compensate_ddl_end()) {
        TRANS_LOG(DEBUG, "<ObTxDirectLoadIncLog> need compensate ddl end log", K(ret), K(tmp_ret),
                  K(submit_log_type), K(trans_id_), K(iter->first));
        if (compensate_ddl_end_fail_cnt <= MAX_COMPENSATE_FAIL_CNT
          ...
          if (OB_ISNULL(extra_cb)) {
            ...
            TRANS_LOG(WARN, "alloc memory failed", K(ret), K(trans_id_),
                      K(submit_log_type));
          } else {
          ...
          if (OB_TMP_FAIL(ret)) {
            // do nothing
          } else if (OB_TMP_FAIL(extra_cb->init(iter->first.get_batch_key()))) {
            ...
            if (tmp_ret != OB_TX_NOLOGCB) {
              TRANS_LOG(WARN, "submit direct load inc commit log failed", K(ret), K(inc_commit_log),
                        KPC(extra_cb), K(submitted_scn), K(trans_id_));
            }
            extra_cb->~ObDDLIncCommitClogCb();
            mtl_free(extra_cb);
          } else {}
        ...
        TRANS_LOG(WARN, "push back unused batch key failed", K(ret), K(tmp_ret), K(submit_log_type),
                  K(trans_id_), K(iter->first));
      } else {
        TRANS_LOG(INFO, "<ObTxDirectLoadIncLog> Try to remove unused batch info", K(ret),
                  K(tmp_ret), K(submit_log_type), K(trans_id_), K(iter->first),
    ...
    if (OB_TMP_FAIL(exec_info_.dli_batch_set_.remove_unlog_batch_info(unused_batch_key_array))) {
      TRANS_LOG(WARN, "remove unlog batch_info failed", K(ret), K(tmp_ret), K(submit_log_type),
                K(trans_id_), K(unused_batch_key_array));
    }
    ...
    if (OB_SUCC(ret) && need_ddl_end_count > 0) {
      if (ObTxLogTypeChecker::is_state_log(submit_log_type)
        ...
        TRANS_LOG(ERROR,
                  ...
                  K(ret), K(tmp_ret), K(submit_log_type), K(trans_id_), K(need_ddl_end_count), K(ObTimeUtility::fast_current_time() - start_ts));
      } else {
        ret = OB_TRANS_CANNOT_BE_KILLED;
        TRANS_LOG(WARN,
                                      ...
                                      ObRedoLogSubmitHelper *helper)
{
  ...
  if(OB_FAIL(ret)) {
    TRANS_LOG(WARN, "after submit log failed", K(ret), K(trans_id_), K(exec_info_), K(*log_cb));
  } else {
#ifndef NDEBUG
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_UNLIKELY(replay_completeness_.is_unknown())) {
      ...
      if (OB_FAIL(set_replay_completeness_(replay_continous, log_ts_ns))) {
        TRANS_LOG(WARN, "set replay completeness failed", KR(ret), K(trans_id_), KP(this));
      }
    }
  }
  return ret;
 ...
 */
int ObTxCtx::check_and_merge_redo_lsns_(const palf::LSN &offset)
{
  int ret = OB_SUCCESS;
                                        ...
                                        const bool is_force_kill)
{
  ...
  if (OB_FAIL(errsim_notify_mds_())) {
    TRANS_LOG(WARN, "notify mds errsim", K(ret), K(trans_id_), K(notify_type), K(log_ts),
                                 ...
                                 const concurrent_control::ObWriteFlag &write_flag)
{
  ...
  last_request_ts_ = ObClockGenerator::getClock();
  TRANS_LOG(TRACE, "start_access", K(ret), K(data_scn), KPC(this));
...
int ObTxCtx::try_alloc_retain_ctx_func_()
{
  int ret = OB_SUCCESS;
...
int ObTxCtx::try_gc_retain_ctx_func_()
{
  int ret = OB_SUCCESS;
...
int ObTxCtx::insert_into_retain_ctx_mgr_(RetainCause cause,
                                                const SCN &log_ts,
                                                palf::LSN lsn,
                                                bool for_replay)
{
...
int ObTxCtx::do_local_tx_end_(TxEndAction tx_end_action)
{
  ...
  if (tx_end_action != TxEndAction::KILL_TX_FORCEDLY
    ...
    case TxEndAction::COMMIT_TX: {
      if (runtime_state_.is_force_abort()) {
        if (OB_FAIL(compensate_abort_log_())) {
          TRANS_LOG(WARN, "compensate abort log failed", K(ret), K(trans_id_),
                    K(tx_end_action), K(runtime_state_));
        } else {
          ret = OB_TRANS_KILLED;
        }
      } else {
...
void ObTxCtx::report_write_ctx_status_(const int status, const bool check_tx_status)
{
  ...
  if (OB_ISNULL(trans_service_)) {
    ...
    TRANS_LOG(WARN, "report write ctx status fail", K(ret), K(status), KPC(this));
...
inline bool ObTxCtx::is_support_parallel_replay_() const
{
...
inline int ObTxCtx::correct_cluster_version_(uint64_t cluster_version_in_log)
{
  int ret = OB_SUCCESS;
...
inline bool ObTxCtx::is_support_tx_op_() const
{
  return cluster_version_accurate_;
}
