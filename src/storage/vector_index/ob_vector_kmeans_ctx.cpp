...
int ObMultiKmeansExecutor::build(storage::ObInsertMonitor *insert_monitor)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObMultiKmeansExecutor::build_parallel(const common::ObTableID &table_id,
                                         ...
                                         storage::ObInsertMonitor *insert_monitor)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(prepare_splited_arrs(splited_arrs))) {
      ...
      // build_parallel
      storage::ObIVectorIndexRuntime *runtime =
...
int ObIvfPqBuildHelper::build(const common::ObTableID &table_id,
                             const common::ObTabletID &tablet_id,
                             storage::ObInsertMonitor *insert_monitor)
{
  int ret = OB_SUCCESS;
