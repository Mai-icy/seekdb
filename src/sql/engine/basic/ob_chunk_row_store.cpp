...
int ObChunkRowStore::write_file(void *buf, int64_t size)
{
  ...
  if (OB_SUCC(ret) && size > 0) {
    ...
    if (OB_FAIL(SERVER_TMP_FILE_MANAGER.write(io_))) {
                               ...
                               int64_t &tmp_file_size)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    if (0 == read_size
        && OB_FAIL(SERVER_TMP_FILE_MANAGER.get_tmp_file_size(io_.fd_, tmp_file_size))) {
      LOG_WARN("failed to get tmp file size", K(ret));
    } else if (OB_FAIL(SERVER_TMP_FILE_MANAGER.pread(io_, offset, handle))) {
      if (OB_ITER_END != ret) {
