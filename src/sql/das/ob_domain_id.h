...
namespace oceanbase { namespace share { namespace schema { class ObColumnSchemaV2; class ObTableSchema; } } }
...
public:
  ...
  static bool check_table_need_column_ref_in_ddl(const void *table_schema, const schema::ObColumnSchemaV2 *col_schema);
  /* DML Resolver End */
  ...
  static int64_t get_domain_type_by_col_expr(const void *col_expr, schema::ObIndexType index_type);
  /* tsc cg End */
...
private:
  static int get_pq_cids_col_id(const schema::ObTableSchema &index_table_schema, const schema::ObTableSchema &data_table_schema, uint64_t &pq_cids_col_id);
