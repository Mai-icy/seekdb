...
class ObLogCompressorTest : public ::testing::Test
{
public:
...
TEST_F(ObLogCompressorTest, log_priority_array_test)
{
  int ret = OB_SUCCESS;
...
TEST_F(ObLogCompressorTest, base_compressor_test)
{
  int ret = OB_SUCCESS;
  int test_count = 1000;
...
TEST_F(ObLogCompressorTest, syslog_compressor_base_test)
{
  int ret = OB_SUCCESS;
...
TEST_F(ObLogCompressorTest, syslog_compressor_thread_test)
{
  int ret = OB_SUCCESS;

  // init log compressor
  const int clock_ret = ObClockGenerator::get_instance().init();
  ASSERT_TRUE(OB_SUCCESS == clock_ret || OB_INIT_TWICE == clock_ret);
  // loop faster
  LOG_INFO("start to init syslog compressor ");
  ASSERT_EQ(true, OB_LOG_COMPRESSOR.is_inited_);
  strncpy(OB_LOG_COMPRESSOR.syslog_dir_, TEST_DIR, strlen(TEST_DIR));
