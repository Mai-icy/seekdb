...
TEST_F(TestVsagAdaptor, test_hnsw)
{
  ...
  roaring64_bitmap_add_range(r1, 0, num_vectors + inc_num - 32);
...
TEST_F(TestVsagAdaptor, test_hnswsq)
{
  ...
  roaring::api::roaring64_bitmap_add(r1, num_vectors * 10);
  roaring::api::roaring64_bitmap_add(r1, num_vectors * 10 + 1);
  roaring::api::roaring64_bitmap_add(r1, num_vectors * 10 + 2);
...
TEST_F(TestVsagAdaptor, test_hgraph_iter_filter)
{
  ...
  roaring::api::roaring64_bitmap_t* r1 = roaring::api::roaring64_bitmap_create();
  roaring64_bitmap_add_range(r1, 0, 32);
  TestFilter testfilter(r1);
  const float *distances;
...
TEST_F(TestVsagAdaptor, test_hnsw_iter_filter)
{
  ...
  roaring::api::roaring64_bitmap_t* r1 = roaring::api::roaring64_bitmap_create();
  roaring64_bitmap_add_range(r1, num_vectors, num_vectors + 32);
  TestFilter testfilter(r1);
  const float *distances;
  const char* extra_infos = nullptr;
