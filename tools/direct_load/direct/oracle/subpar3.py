...
def noEmpty_load(host,port,user,tenant,password,directory):
    load_sql = "load data /*+ parallel(10) direct(true, 0) */ infile '"+directory+"/TEST.2.0.csv,"+directory+"/TEST.3.0.csv,"+directory+"/TEST.4.0.csv'  into table subpar3 fields terminated by '|' enclosed by '''' lines starting by '' terminated by '\\n'"
    cmd_str = """ obclient -h%s -P%s -u%s@%s -c  -p%s -e "%s;" """ % (host, port,user,tenant, password, load_sql)
    result = commands.getstatusoutput(cmd_str)
    print("非空表导入多文件结果：")
    error(result,load_sql)
...
def Null_value(host,port,user,tenant,password,directory):
    load_sql = "load data /*+ parallel(10) direct(true, 0) */ infile '"+directory+"/null.csv' into table subpar3 fields terminated by '|' enclosed by '''' lines starting by '' terminated by '\\n'"
    cmd_str = """ obclient -h%s -P%s -u%s@%s -c  -p%s -e "%s;" """ % (host, port,user,tenant, password, load_sql)
    result = commands.getstatusoutput(cmd_str)
...
def FalseType(host,port,user,tenant,password,directory):
    ...
    error(result,load_sql)
    load_sql = "load data /*+ parallel(10) direct(true, 0) */ infile '"+directory+"/false_type.csv'  into table subpar3 fields terminated by '|' enclosed by '''' lines starting by '' terminated by '\\n'"
    cmd_str = """ obclient -h%s -P%s -u%s@%s -c  -p%s -e "%s;" """ % (host, port,user,tenant, password, load_sql)
    result = commands.getstatusoutput(cmd_str)
    print("错误类型导入，预期报错：")
    success(result,load_sql)
